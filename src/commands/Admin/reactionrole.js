...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      return interaction.editReply({ content: 'Could not find that message in this channel, or the emoji is invalid.' });
    }

    await addReactionRole(messageId, emoji, role.id);
    return interaction.editReply({ content: `Reacting with ${emoji} on that message now assigns the **${role.name}** role.` });
  },
};